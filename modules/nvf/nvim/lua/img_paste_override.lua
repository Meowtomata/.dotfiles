...
function M.get_clipboard_img_type()
  local this_os = api.get_os()
  if this_os ~= api.OSType.Linux and this_os ~= api.OSType.FreeBSD then
    -- For other OSes, maintain original behavior
    local check_cmd = get_clip_check_command()
    local result_string = vim.fn.system(check_cmd)
    local content = vim.split(result_string, "\n")
    if this_os == api.OSType.Darwin then
      if string.sub(content[1], 1, 9) == "iVBORw0KG" then
        return "png"
      end
    elseif this_os == api.OSType.Windows or this_os == api.OSType.Wsl then
      if content ~= nil and #content > 0 and content[1] ~= "" then
        return "png"
      end
    end
    return nil
